...
public class FeeCalculator {
	...
	public BigDecimal calculateTotalPrice(Set<PromoOption> selectedPromoOptions, Set<Integer> categoryPath) {
		Map<PromoOption, BigDecimal> calculatedFees = calculateFees(selectedPromoOptions, categoryPath);
		return totalFees(calculatedFees);
	}

	private Map<PromoOption, BigDecimal> calculateFees(Set<PromoOption> selectedPromoOptions,
		...
		while (it.hasPrevious()) {
			...
			if (priceListForCategory != null) {
				addFeesForPromoOptionsDefinedInPriceList(selectedPromoOptions, priceListForCategory,
						feesForSelectedPromoOptions);
			}
		}
		return feesForSelectedPromoOptions;
	}

	private void addFeesForPromoOptionsDefinedInPriceList(Set<PromoOption> selectedPromoOptions,
			PriceList priceListForCategory, Map<PromoOption, BigDecimal> feesForSelectedPromoOptions) {
